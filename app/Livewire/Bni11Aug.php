 ...
 */
class Bni11Aug extends Component
{
    use WithFileUploads;
    ...
    public function rules(): array
    {
        ...
        if ($this->showsIndustryConnectField()) {
            $rules['industries'] = ['nullable', 'array'];
            $rules['industries.*'] = [Rule::in($this->availableIndustryValues())];
        }
    ...
    public function messages(): array
    {
        return [
            ...
            'industries.array' => '* mandatory',
            'industries.*.in' => '* mandatory',
    ...
    /**
     * Determine whether the visitor-only industry connect field should be shown.
     */
    public function showsIndustryConnectField(): bool
    {
        return $this->type === VisitorType::VISITOR->value;
    }

    /**
     * Load member categories used as industry connect options.
     *
     * @return Collection<int, MemberCategory>
     */
    #[Computed]
    public function memberCategories(): Collection
    {
    ...
    {
        return view('livewire.bni11-aug');
    }
    ...
    /**
     * Return member category names accepted as industry connect options.
     *
     * @return array<int, string>
     */
    protected function availableIndustryValues(): array
    {
        return $this->memberCategories->pluck('name')->all();
    }
    ...
    protected function visitorData(): array
    {
        ...
        if ($this->isSubstituteResponse()) {
            $meta['substituted_by'] = $this->substituted_by;
        }

        if ($this->showsIndustryConnectField() && $this->industries !== []) {
            $meta['connect_industry'] = implode(', ', $this->industries);
        }
